# Play Console → App content

Public page: https://thetruth.currentflowconsulting.org/play-console

Use these exact answers. The listing is free. Do not add ads or in-app products.

## Policy → App content → Privacy policy

### Privacy policy URL

**https://thetruth.currentflowconsulting.org/privacy**

## Policy → App content → Ads

### Does your app contain ads?

**No**

### Third-party ad SDKs, banners, or native ads

**No**

## Policy → App content → App access

### All functionality available without special access?

**Yes**

### Any part of the app restricted (login, password, membership)?

**No**

### Instructions for reviewers

**Reading, search, and study work without an account. Sign-in is optional. Do not use the platform owner page.**

## Policy → App content → Advertising ID

### Does the app use the advertising ID?

**No**

### AD_ID permission

**Removed in the Android manifest.**

## Policy → App content → News apps

### Is this a news or magazine app?

**No**

### Category News and Magazines

**Do not select.**

## Policy → App content → COVID-19

### Is this a COVID-19 contact-tracing or status app?

**No**

## Policy → App content → Government apps

### Is this a government app?

**No**

### Developed by or on behalf of a government agency?

**No**

## Policy → App content → Financial features

### Does the app provide financial features?

**No**

### Banking, loans, trading, crypto wallets, or payments?

**No**

### The listing is free. There is no checkout.

**Yes — free, no checkout.**

## Policy → App content → Health

### Does the app have health features?

**No**

### Medical device, diagnosis, treatment, or Health Connect?

**No**

## Policy → App content → AI-generated content

### Is a generative-AI chatbot a central feature?

**No**

### Does the app generate images, voice, or video from prompts?

**No**

### Do Research, Investigate, Assistant, and Word Study use a paid AI API?

**No**

_Those tools search texts stored in the app. They do not require OpenAI or any paid API._

### Store listing graphics AI label

**Do not check. Assets are not generative AI.**

## Policy → App content → Photos, videos, and other permissions

### Does the app request Photos or Videos permission?

**No**

### Camera, microphone, contacts, SMS, call log, or location?

**No**

### Foreground services / Health Connect / VPN?

**No**

### Android permissions in the Play wrapper

**INTERNET only. Advertising ID is removed.**

## Policy → App content → Target audience and content

### Target age groups

**13–15, 16–17, and 18+**

### Children under 13

**Do not select**

### Appeals to children

**No**

### Designed for Families / Families policy

**No. Do not enroll.**

### Store presence for children

**Not designed for children**

### Why these ages

**The app is a Scripture reference tool. It is not a children’s game.**

## Policy → App content → Data safety

### Does the app collect or share any required user data types?

**Yes — see the types below.**

### Does the app collect required user data to work?

**No. Reading works without an account.**

### Data encrypted in transit?

**Yes (HTTPS)**

### Users can request that data be deleted?

**Yes — https://thetruth.currentflowconsulting.org/account**

### Data sold or used for advertising?

**No**

### Independent security review?

**No**

### Committed to Play Families Policy?

**No**

### App info and performance → Other app performance data

**Collected: Yes. Shared: No. Optional: Yes. Ephemeral: No. Purpose: Analytics.**

### Device or other IDs

**Collected: Yes. Shared: No. Optional: Yes. Ephemeral: No. Purpose: Analytics (random install device id).**

### Personal info → Email address, Name

**Collected by our servers: No. Shared: Yes, with Google, only if Continue with Google. Optional. Purpose: App functionality.**

### Photos and videos → Photos

**Collected by our servers: No. Shared: Yes, Google profile picture URL only if Continue with Google. Optional. The app does not request the Android photos permission.**

### Location, financial, health, messages, files, audio, advertising IDs

**Do not declare.**

### Notes and highlights

**Do not declare. They stay on the device.**

## Policy → App content → Content ratings (IARC)

### IARC email

**owner@thetruth.currentflowconsulting.org**

### Category

**All Other App Types (Utility / Productivity / Reference). Not a game.**

### Does the app contain depictions of violence?

**Yes**

### Is the violence interactive gameplay or a combat game?

**No**

### Cartoon or fantasy violence?

**No**

### Realistic or graphic violence toward humans or animals?

**Yes — textual accounts only, not images.**

### Blood, gore, or dismemberment as images or gameplay?

**No**

### Sexual violence as interactive or illustrated content?

**No**

### Sexual content or nudity images?

**No**

### Sexual themes in text?

**Yes, infrequent biblical narrative. No pornography.**

### Profanity or crude language?

**Yes — some strong or archaic wording in stored texts.**

### Alcohol, tobacco, or drugs sold or promoted?

**No**

### References to wine or similar in stored texts?

**Yes, infrequently.**

### Real-money or simulated gambling?

**No**

### Users can interact or chat with each other?

**No**

### Public user-generated content / social feed?

**No**

### Shares the user’s location?

**No**

### Digital purchases or in-app products?

**No**

### Unrestricted internet / open web browser?

**No. TWA is locked to this site.**

### Primarily directed at children under 13?

**No**

### Expected rating

**Teen. Not Designed for Families.**

